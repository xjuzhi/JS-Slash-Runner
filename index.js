...
async function renderMessagesInIframes() {
    ...
    messagesToProcess.forEach((message) => {
        ...
        // 查找 <code> 标签
        const codeElements = mesTextContainer.querySelectorAll("code");
        ...
        codeElements.forEach((codeElement) => {
            ...
            iframe.id = `message-iframe-${messageId}`;
        ...
        `;
            ...
            // 清空现有的 mes_text 并插入 iframe
            mesTextContainer.innerHTML = "";
            mesTextContainer.appendChild(iframe);
        });
    });
}
...
async function loadSettings() {
    ...
    $("#process_depth").val(
        extension_settings[extensionName].process_depth || 0
    );
...
function onExtensionInput(event) {
    ...
    if (settingId === "activate_setting") {
        ...
        if (isEnabled) {
            ...
            context.reloadCurrentChat(); // 如果扩展被禁用，恢复视图
        }
