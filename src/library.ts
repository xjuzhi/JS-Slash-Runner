...
function initializeLibraries(): void {
  const filterScriptFromRegex = (script: RegexScriptData) => script.scriptName.startsWith("库-");
  const isEnabled = (script: RegexScriptData) => !script.disabled;
  const toName = (script: RegexScriptData) => script.scriptName.replace('库-', '');
