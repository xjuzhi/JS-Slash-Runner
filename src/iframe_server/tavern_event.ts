...
type TavernCallback = (...args: any[]) => Promise<any>;

let iframe_listener_event_callback_map: Map<string, Map<string, Map<TavernEventType, TavernCallback>>> = new Map();

interface IframeTavernNormalMessage {
  request: 'iframe_tavern_on' | 'iframe_tavern_make_last' | 'iframe_tavern_make_first' | 'iframe_tavern_once' | 'iframe_tavern_remove_listener';
  event_type: TavernEventType;
  listener_name: string;
}

interface IframeTavernClearEventMessage {
  request: 'iframe_tavern_clear_event';
  event_type: TavernEventType;
}

interface IframeTavernClearListenerMessage {
  request: 'iframe_tavern_clear_listener';
  listener_name: string;
}

interface IframeTavernClearAllMessage {
...
function unpack(event: MessageEvent<IframeTavernNormalMessage>) {
  return {
...
function tryGetTavernCallback(event: MessageEvent<IframeTavernNormalMessage>): Callback | undefined {
...
function removeTavernCallback(event: MessageEvent<IframeTavernNormalMessage>): void {
  const data = unpack(event);
  // @ts-ignore 2345
  iframe_listener_event_callback_map.get(data.iframe_name).get(data.listener_name).delete(data.event_type);
}

function makeTavernCallback(event: MessageEvent<IframeTavernNormalMessage>, once: boolean): TavernCallback {
  const data = unpack(event);
...
const event_handlers = {
  iframe_tavern_on: (event: MessageEvent<IframeTavernNormalMessage>): void => {
    const data = unpack(event);
  ...
  iframe_tavern_make_last: (event: MessageEvent<IframeTavernNormalMessage>): void => {
  ...
  iframe_tavern_make_first: (event: MessageEvent<IframeTavernNormalMessage>): void => {
    const is_listening = tryGetTavernCallback(event) !== undefined;
  ...
  iframe_tavern_once: (event: MessageEvent<IframeTavernNormalMessage>): void => {
    const data = unpack(event);
  ...
  iframe_tavern_remove_listener: (event: MessageEvent<IframeTavernNormalMessage>): void => {
    const data = unpack(event);
  ...
  iframe_tavern_clear_event: (event: MessageEvent<IframeTavernClearEventMessage>): void => {
  ...
  iframe_tavern_clear_listener: (event: MessageEvent<IframeTavernClearListenerMessage>): void => {
    const iframe_name = getIframeName(event);
  ...
  iframe_tavern_clear_all: (event: MessageEvent<IframeTavernClearAllMessage>): void => {
    const iframe_name = getIframeName(event);
...
function clearIframeTavernEventListeners(iframe_name: string): void {
...
async function handleTavernEvent(event: MessageEvent<IframeTavernMessage>): Promise<void> {
