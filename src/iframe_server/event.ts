...
interface IframeEventEntryMessage {
...
interface IframeEventEmitMessage {
...
interface IframeEventClearEventMessage {
  request: 'iframe_event_clear_event';
  event_type: EventType;
}

interface IframeEventClearListenerMessage {
  request: 'iframe_event_clear_listener';
  listener_uid: number;
  listener_string: string;
}

interface IframeEventClearAllMessage {
  request: 'iframe_event_clear_all',
}

type IframeEventMessage = IframeEventEntryMessage | IframeEventEmitMessage | IframeEventClearEventMessage | IframeEventClearListenerMessage | IframeEventClearAllMessage;

// TODO: don't repeat this in all files
function getIframeName(event: MessageEvent<IframeEventMessage>): string {
  const window = event.source as Window;
  return window.frameElement?.id as string;
}

function unpack(event: MessageEvent<IframeEventEntryMessage>) {
  return {
...
function tryGetEventCallback(event: MessageEvent<IframeEventEntryMessage>): Function | undefined {
...
function removeEventCallback(event: MessageEvent<IframeEventEntryMessage>): void {
  const data = unpack(event);
  // @ts-ignore 2345
...
const event_handlers = {
  iframe_event_on: async (event: MessageEvent<IframeEventEntryMessage>) => {
    const data = unpack(event);
  ...
  iframe_event_make_last: async (event: MessageEvent<IframeEventEntryMessage>) => {
  ...
  iframe_event_make_first: async (event: MessageEvent<IframeEventEntryMessage>) => {
    const is_listening = tryGetEventCallback(event) !== undefined;
  ...
  iframe_event_once: async (event: MessageEvent<IframeEventEntryMessage>) => {
    const data = unpack(event);
  ...
  iframe_event_emit: async (event: MessageEvent<IframeEventEmitMessage>) => {
  ...
  iframe_event_remove_listener: async (event: MessageEvent<IframeEventEntryMessage>) => {
    const data = unpack(event);
  ...
  iframe_event_clear_event: async (event: MessageEvent<IframeEventClearEventMessage>) => {
  ...
  iframe_event_clear_listener: async (event: MessageEvent<IframeEventClearListenerMessage>) => {
    const iframe_name = getIframeName(event);
  ...
  iframe_event_clear_all: (event: MessageEvent<IframeEventClearAllMessage>): void => {
    const iframe_name = getIframeName(event);
...
async function handleEvent(event: MessageEvent<IframeEventMessage>): Promise<void> {
